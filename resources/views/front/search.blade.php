...
@section('seo_title', page_content('search', 'page_title', app()->getLocale()) . ': ' . $query . ' - ' . ($siteSetting->site_name ?? 'Portfolio'))
@section('meta_description', 'Search results for ' . $query . '. Find projects, blog posts, and services matching your query.')

@section('content')
<section class="section-padding section-1" style="padding-top: 8rem;">
    <div class="container">
            ...
            <div class="col-lg-6">
                <form action="{{ route('search') }}" method="GET" class="d-flex gap-2">
                    <input type="text" name="q" class="form-control form-control-lg" 
                           placeholder="{{ page_content('search', 'form_placeholder', app()->getLocale()) }}" 
        ...
        @if($totalResults > 0)
            <div class="row g-4">
                ...
                @if($projects->isNotEmpty())
                    <div class="col-12">
                        <h4 class="mb-4"><i class="fa-solid fa-folder-open text-primary-custom me-2"></i>{{ page_content('search', 'projects_title', app()->getLocale()) }} ({{ $projects->count() }})</h4>
        ...
        @else
            <div class="text-center py-5">
                ...
                <h4 class="mt-4">{{ page_content('search', 'empty_title', app()->getLocale()) }}</h4>
                <p class="text-muted">{{ page_content('search', 'empty_text', app()->getLocale()) }}</p>
                <div class="d-flex justify-content-center gap-3 mt-4">
                    <a href="{{ route('projects.index') }}" class="btn btn-outline-primary">{{ page_content('search', 'empty_projects', app()->getLocale()) }}</a>
                    <a href="{{ route('blog.index') }}" class="btn btn-outline-primary">{{ page_content('search', 'empty_blogs', app()->getLocale()) }}</a>
                    <a href="{{ route('services') }}" class="btn btn-outline-primary">{{ page_content('search', 'empty_services', app()->getLocale()) }}</a>
                </div>
            </div>
        @endif
    </div>
