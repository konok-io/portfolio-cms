...
<div class="admin-page-header">
    <div>
        <h1>Edit Project</h1>
        <p class="admin-breadcrumb mb-0"><a href="{{ route('admin.projects.index') }}">Projects</a> / Edit</p>
    </div>
</div>
                ...
                <div class="card-body-custom">
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="description" id="description" class="form-control" rows="8">{{ old('description', $project->description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="row g-2 mb-3">
                            @foreach($project->gallery as $image)
                                <div class="col-3 position-relative">
                                    <img src="{{ $image->image_url }}" class="img-fluid rounded-2" style="aspect-ratio:1/1;object-fit:cover;width:100%;">
                                    <form action="{{ route('admin.projects.gallery.destroy', $image) }}" method="POST" class="position-absolute top-0 end-0 m-1" data-confirm-delete="Remove this gallery image?">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger rounded-circle" style="width:26px;height:26px;padding:0;line-height:1;"><i class="fa-solid fa-xmark small"></i></button>
                                    </form>
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="admin-card mb-3">
                ...
                <div class="card-body-custom">
                    ...
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $project->is_active) ? 'checked' : '' }}>
                        <label class="form-check-label" for="is_active">Active (visible on site)</label>
                    </div>
                    <label class="form-label-admin">Sort Order</label>
                    <input type="number" name="sort_order" min="0" class="form-control" value="{{ old('sort_order', $project->sort_order) }}">
                </div>
            </div>
